...
class BWTrainDataset(BaseDataset):
    def __init__(self, root_dataset, odgt, opt, batch_per_gpu=1, **kwargs):
        super(BWTrainDataset, self).__init__(odgt, opt, **kwargs)
        self.root_dataset = root_dataset
        # down sampling rate of segm labe
        self.segm_downsampling_rate = opt.segm_downsampling_rate
        self.batch_per_gpu = batch_per_gpu
    ...
    def __getitem__(self, index):
        ...
        for i in range(self.batch_per_gpu):
            ...
            # image transform, to torch float tensor 1xHxW
            img = self.img_transform(img)
...
class BWTestDataset(BaseDataset):
    def __init__(self, odgt, opt, **kwargs):
        super(BWTestDataset, self).__init__(odgt, opt, **kwargs)
...
class BWValDataset(BaseDataset):
    def __init__(self, root_dataset, odgt, opt, **kwargs):
        super(BWValDataset, self).__init__(odgt, opt, **kwargs)
        self.root_dataset = root_dataset
